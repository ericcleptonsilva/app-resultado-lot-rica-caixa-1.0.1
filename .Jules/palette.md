...
## 2026-10-25 - Accessible Ligature Icons
**Learning:** Decorative font icons (such as Google Material Icons) that use ligature text (e.g., 'auto_fix_high') are erroneously announced by screen readers if not hidden.
**Action:** Always include `aria-hidden="true"` on these elements to prevent screen readers from reading the raw ligature text to users.
