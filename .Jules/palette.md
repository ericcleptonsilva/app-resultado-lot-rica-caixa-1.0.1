...
## 2026-10-25 - Hiding Decorative Icons and Disabled Button States
**Learning:** Screen readers announce ligature text of material icons (like "auto_fix_high") leading to confusion, and disabled loading buttons lack visual cues without style changes.
**Action:** Always add `aria-hidden="true"` to decorative `<span className="material-icons">` and dynamically apply `cursor: 'not-allowed'` and `opacity: 0.7` to disabled loading buttons.
