...
## 2024-03-24 - Hide Decorative Icons from Screen Readers
**Learning:** Material Icons implemented as `<span className="material-icons">text</span>` cause screen readers to announce the ligature text (e.g., "auto_fix_high") which is confusing and non-semantic for users.
**Action:** Always append `aria-hidden="true"` to non-semantic decorative ligature icons throughout the app to prevent redundant and confusing screen reader announcements.
