...
## 2026-10-26 - Material Icons Screen Reader Announcement
**Learning:** Decorative font icons (such as Google Material Icons) that use ligature text (e.g., 'auto_fix_high') will be erroneously announced by screen readers as the literal ligature text if not hidden.
**Action:** Always include `aria-hidden="true"` on decorative icons, particularly ligature-based font icons, to prevent confusing announcements for screen reader users.
