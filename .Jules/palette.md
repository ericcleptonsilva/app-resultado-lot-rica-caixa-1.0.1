...
## 2026-10-25 - Ligature Icons Accessibility
**Learning:** Decorative font icons (such as Google Material Icons) that use ligature text (e.g., 'auto_fix_high') are read aloud by screen readers, causing confusion.
**Action:** Always include `aria-hidden="true"` on the `<span>` or `<i>` tag containing the ligature to prevent screen readers from erroneously announcing the ligature text to users.
