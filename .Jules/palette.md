...
## 2026-10-25 - Decorative Font Icons Accessibility
**Learning:** Decorative font icons that use ligature text (e.g., `<span className="material-icons">auto_fix_high</span>`) are erroneously announced by screen readers as the literal ligature text, confusing users.
**Action:** Always add `aria-hidden="true"` to ligature-based font icon elements so screen readers ignore them, relying instead on proper text labels or ARIA labels on their parent interactive elements.
