...
## 2024-04-03 - Accessible Form Validation
**Learning:** Using click-triggered alerts for missing form requirements blocks users and hides conditions. Relying purely on visual disabled styles without explanations leaves users confused about why they can't proceed.
**Action:** Primary action buttons (e.g., 'Salvar') must use the native `disabled` attribute when conditions are not met, paired with a `title` attribute to explain the reason.
