...
## 2026-10-25 - Delight vs Utility
**Learning:** "Delight" features like AI predictions are fun but lose value if they don't connect to the core workflow. Users shouldn't have to memorize predictions to use them.
**Action:** Always bridge "delight" features to core utility by providing direct action buttons (e.g., "Jogar Agora") that automatically populate forms and navigate users to the relevant workflow.

## 2026-10-25 - Accessible Async Content
**Learning:** Dynamically loaded content like AI predictions can be completely missed by screen reader users if not properly announced when it appears.
**Action:** Use `aria-live="polite"` on containers that load dynamic async content so that screen readers proactively announce the updates without interrupting the user.
